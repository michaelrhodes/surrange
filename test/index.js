...
run('Even surround values work nicely', function(test) {
  ...
  test.equal(result.join(), expected.join())
  test.end()
})
...
run('Positive flag pushes negative numbers up from 1', function(test) {
  ...
  test.equal(result.join(), expected.join(), 'with negative number')
 
  expected = [1, 2, 3, 4, 5, 6, 7, 8]
  result = surrange(4, 7, true)
  
  test.equal(result.join(), expected.join(), 'with negative low bound')
